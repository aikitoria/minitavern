...
**Steered regeneration** (`POST /api/messages/:id/regenerate`): like advancing past the last sibling, but the prompt is snapshotted at route time via `promptOverride` (`buildSteeredPrompt`) with the instruction rendered into the conversation's resolved `steerTemplate` (`resolveSteerTemplate`: character inline `customTemplate` → character `templateId` → global default template → `DEFAULT_STEER_TEMPLATE`; `{{instruction}}` slot) as a trailing system message — it steers that one generation only and never enters message history.
