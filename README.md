...
- Avatar generation: characters and personas get a "Generate" button next to "Change avatar" — a popup streams the model's portrait prompt (configurable `avatarPrompt` template with `{{char}}`/`{{description}}`/… macros), renders it live, and lets you regenerate, edit the text and re-render, save, or cancel. Prompt and workflow are configurable in Settings → Tools → Image Generation.
